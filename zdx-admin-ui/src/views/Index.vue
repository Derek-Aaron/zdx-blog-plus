<template>
	<div class="dashboard-container">
			...
			<el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
				<div class="card-panel">
					<div class="card-panel-icon-wrapper icon-view">
						<svg-icon icon-class="visits" size="4em" class-tagName="card-panel-icon"/>
					</div>
			...
			<el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
				<div class="card-panel">
					<div class="card-panel-icon-wrapper icon-money">
						<svg-icon icon-class="article" size="3rem" class-tagName="card-panel-icon"/>
					</div>
					<div class="card-panel-description">
						<div class="card-panel-text">
							文章量
		...
		<el-row class="data-card" style="margin-bottom:32px;">
			...
			<calendar-heatmap style=" width: 100%; margin-top: 0.5rem" :values="articleStatisticsList"
							  :end-date="new Date()"
...
<script setup>
import {getBlogInfo} from "@/api/base";
...
onMounted(() => {
	getBlogInfo().then((res) => {
		viewCount.value = parseInt(res.data.viewCount)
		messageCount.value = parseInt(res.data.messageCount)
		userCount.value = parseInt(res.data.userCount)
		articleCount.value = parseInt(res.data.articleCount)
		articleStatisticsList.value = res.data.articleStatisticsList;
		if (res.data.tagVOList != null) {
			tagList.value = res.data.tagVOList;
			tagLoad.value = true;
		}
		if (res.data.articleRankVOList) {
			res.data.articleRankVOList.forEach((item) => {
				ariticleRank.series[0].data.push(item.viewCount);
				ariticleRank.xAxis.data.push(item.title);
			});
		}
		if (res.data.categoryVOList != null) {
			res.data.categoryVOList.forEach((item) => {
