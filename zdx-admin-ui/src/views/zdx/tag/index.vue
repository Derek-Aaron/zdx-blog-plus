...
const handleUpdate = (row) => {
    ...
    dialog.value = true
    title.value = '编辑标签 【' + entity.value.name + '】'
}
    ...
    <div class="app-container">
        ...
        <el-form :model="queryParams.params" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
            <el-form-item label="标签名" prop="name">
                <el-input v-model="queryParams.params.name" placeholder="请输入标签名" clearable style="width: 240px"
                    @keyup.enter="pageTag" @clear="pageTag" />
            </el-form-item>
            <el-form-item>
                <el-button type="primary" icon="Search" @click="pageTag">搜索</el-button>
        ...
        <zdx-dialog :dialog="dialog" :title="title" @close="dialog = false">
            <template #content>
                <el-form :model="entity" :rules="rules" ref="formRef" label-width="80px">
                    <el-form-item label="标签名" prop="name">
                        <el-input v-model="entity.name" clearable placeholder="请输入标签名" />
                    </el-form-item>
                </el-form>
            </template>
            <template #footer>
