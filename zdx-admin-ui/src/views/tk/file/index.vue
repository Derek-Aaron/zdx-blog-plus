...
<template>
    <div class="app-container">
        ...
        <el-table v-loading="loading" :data="fileList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="文件名" align="center" key="name" prop="name" show-overflow-tooltip v-if="columns[1].visible" />
            <el-table-column label="文件类型" align="center" key="bucketName" prop="bucketName" v-if="columns[2].visible">
                <template #default="scope">
                    <zdx-dict-tag :options="zdx_file_type" :value="scope.row.bucketName" />
                </template>
            </el-table-column>
            <el-table-column label="文件路径" align="center" key="bucketName" prop="bucketName" show-overflow-tooltip v-if="columns[3].visible" />
            <el-table-column label="文件大小" align="center" key="size" prop="size" show-overflow-tooltip v-if="columns[4].visible" />
            <el-table-column label="创建时间" align="center" key="createTime" prop="createTime" show-overflow-tooltip v-if="columns[5].visible" />
        ...
        <zdx-dialog :dialog="dialog" title="上传文件" @close="dialog = false">
            <template #content>
                <el-upload class="upload-demo" drag action="/api/zdx.file/upload" :headers="{
