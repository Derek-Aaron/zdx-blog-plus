				   ...
				   :transform="getWeekPosition(weekIndex)">
					<template v-for="(day, dayIndex) in week" :key="dayIndex">
						<rect class="vch__day__square" v-if="day.date < now" :rx="round" :ry="round"
							  ...
							  :data-tippy-content="tooltipOptions(day)" :color="day.colorIndex" @click="$emit('dayClick', day)" />
					</template>
				</g>
			</g>
		</svg>
...
import { defineComponent, nextTick, onBeforeUnmount, onMounted, ref, toRef, toRefs, watch } from 'vue';
...
export default /*#__PURE__*/defineComponent({
	...
	setup(props) {
		...
		const SQUARE_BORDER_SIZE = Heatmap.SQUARE_SIZE / 5,
			...
			svg = ref(),
			now = ref(new Date()),
			heatmap = ref(new Heatmap(props.endDate, props.values, props.max)),
			...
			lo = ref({}),
			rangeColor = ref(props.rangeColor || (props.darkMode ? Heatmap.DEFAULT_RANGE_COLOR_DARK : Heatmap.DEFAULT_RANGE_COLOR_LIGHT));
		...
		watch(
			...
			() => {
				heatmap.value = new Heatmap(props.endDate, props.values, props.max);
