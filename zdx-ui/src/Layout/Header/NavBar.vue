			...
			<div v-else class="menu-item dropdown">
				...
				<ul class="submenu">
					<li class="subitem" v-for="submenu of menu.children" :key="submenu.name"
						...
						<router-link v-if="submenu.path" class="link" :to="submenu.path">
							<svg-icon :icon-class="submenu.icon"></svg-icon>
							{{ submenu.name }}
						</router-link>
						<a v-else class="link" :href="submenu.url">
							<svg-icon :icon-class="submenu.icon"></svg-icon>
							{{ submenu.name }}
						</a>
					</li>
	...
	},
	{
		...
		children: [
			...
			},
			{
				name: "源码",
				icon: "talk",
				url: "https://gitee.com/zhaodengxuan/zdx-blog-plus"
			},
			{
				name: "后台",
				icon: "album",
				url: 'https://admin.zhaodengxuan.top'
			}
		]
	},
];
